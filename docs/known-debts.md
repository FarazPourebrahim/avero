...
| A11y | No manual screen-reader pass has been done. The automated coverage is axe in every unit test plus the browser axe suite over every story in both directions, which catches missing names, roles and states but not how a screen reader actually announces a component. | A component can pass axe and still be confusing to a screen-reader user — reading order, verbosity, announcement of live regions and of the Jalali date picker in particular. | Phase 12 item, needs a human: NVDA + Firefox on Windows and VoiceOver + Safari on macOS, over every interactive component. Fix or log each finding here. |
| A11y | No recorded keyboard-only walkthrough of the six example templates. Keyboard behaviour is covered per component by unit tests and the forms Playwright spec, but not end to end across a whole page. | A trap or a wrong tab order that only appears when components are composed would not be caught. | Phase 12 item, needs a human: tab through each `Templates/*` story in both directions and record it. |
...
| Release | The `avero` npm organisation is not reserved (O-06), so nothing is published, provenance is unproven end to end and 2FA cannot be enabled. `.github/workflows/release.yml` is written and publishes with `NPM_CONFIG_PROVENANCE`, but has never run. | The last supply-chain item in `docs/SECURITY.md` cannot be ticked, and Phase 13 is blocked. | Action for the user: create the `avero` npm organisation, enable 2FA, add `NPM_TOKEN` to the repository secrets. Then the release workflow can run. |
...
Redefine these after importing `@avero/tokens/theme.css` and every listed pairing moves with them.
