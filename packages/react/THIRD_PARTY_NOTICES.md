...
`src/icons/internalIcons.tsx` vendors the Lucide glyphs that Avero components render internally (arrow, chevron, star, zoom, heart, share, user and close icons), so `lucide-react` stays an optional peer dependency.
