...
`src/icons/internalIcons.tsx` vendors the Lucide glyphs that Avero components render internally (arrow, chevron, copy, star, zoom, heart, share, message, send, user, flag, shield, warning and close icons), so `lucide-react` stays an optional peer dependency.
