...
| V-11 | Reference screenshot baselines substitute deterministic local placeholders for the 27 images that still point at live third-party hosts (22 on `api.dorlancer.ir` including user avatars, 6 enamad certification marks, 2 Google profile images, 2 site images), and the parity diff masks those regions. Placeholders keep each image's box so layout is unaffected: 23 of the 32 are sized by their own `w-*`/`h-*` classes, and the unsized ones (the enamad seal) use 125×136, measured from the copy the "درباره ما" page saved locally rather than assumed. Image rendering itself (`object-fit`, aspect ratio, radius) stays covered by component tests. | Apply | ✅ Approved 2026-07-05 |
...
- [x] Reference screenshot baselines: the 8 pages served locally, remote images replaced with local placeholders (deviation V-11), captured at 375/768/1280/1536px with Lahzeh loaded — 32 baselines under `apps/replica/baselines/win32/`, per-platform like Storybook's (KD-06) rather than the flat `baselines/reference/` this line originally named. Built by `apps/replica/scripts/build-reference-pages.mjs`, which derives a servable copy into a gitignored `.reference-build/` and never writes to `reference/`. The saved pages ship no font files, so the script copies the licensed Lahzeh faces to the paths each page's `font.css` already expects; without that every baseline would encode fallback-font typography
...
| B-01…B-25 | Blocks (see §7.10) — done: B-01 `ListingCard`, B-02 `ShowcaseCard`, B-03 `OpportunityCard`, B-04 `SuggestionItem`, B-05 `PostListItem`, B-06 `AuthorCard`, B-07 `ProviderCard`, B-08 `CommentSection`, B-09 `ShareBar`, B-10 `ReportAction`/`ReportCard`, B-11 `FilterPanel`, B-12 `PromoBanner`, B-13 `CtaBanner`, B-14 `SplitHero`, B-15 `FeatureGrid`, B-16 `QuickActions`, B-18 `WelcomeCard`, B-22 `ContactMethods`, B-23 `PriceCard`, B-24 `RelatedList`, B-25 `AchievementsPanel` | A | 10 | 🟨 | 🟨 | 🟨 | 🟨 | 🟨 | 🟨 |
