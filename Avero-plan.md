...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 0 | Reference audit and extraction | 🟨 | 8 / 12 | — | M | 2026-06-19 | |
| 1 | Workspace, tooling and CI | ⬜ | 0 / 16 | 0 | M | | |
| 2 | Design tokens and foundations | ⬜ | 0 / 15 | 1 | L | | |
...
**Overall:** 8 / 164 phase-DoD items (≈5%).
...
| *(literal)* `#F59E0B` / `#D97706` | amber | Warning CTA and hover (CSS-only) | Tailwind `amber-500`/`amber-600` |
...
- Vite library mode (Rollup) with `preserveModules`, directive preservation and `.d.ts` via `vite-plugin-dts` or `tsc --emitDeclarationOnly`. Output is ESM with subpath exports per component; `sideEffects` covers `["*.css"]` only.
...
- [ ] `tokens.extracted.json` generated by a script from the CSS (not hand-typed), committed with the script
...
**DoD:**
- [ ] Git initialised or verified with `main` and `dev` branches; work happens on `dev`
- [ ] `pnpm-workspace.yaml` and root `package.json` with per-app cd-scripts only (no bare `dev`), `packageManager` pinned
- [ ] `@avero/config` providing strict `tsconfig` bases, ESLint flat config and Prettier config, consumed by all packages
- [ ] TypeScript strict and `noUncheckedIndexedAccess` on; `tsc -b` at root passes
- [ ] Custom lint rules active: no raw hex/rgb/hsl in `packages/react/src/**`; no physical direction utilities (`pl-`, `pr-`, `ml-`, `mr-`, `left-`, `right-`, `border-l`, `border-r`, `rounded-l`, `rounded-r`, `text-left`, `text-right`); `no-console`
- [ ] Tailwind v4.3.x installed; a smoke component renders a token class in Storybook
- [ ] Vite library build producing ESM + `.d.ts` with preserved `"use client"` directives (verified by a test that inspects the output)
- [ ] Vitest + Testing Library + `vitest-axe` configured, with a sample test passing
- [ ] Playwright configured for visual + axe tests against Storybook and `apps/replica`
- [ ] Storybook (internal) boots with RTL/LTR and locale toolbar toggles
- [ ] `apps/docs` (Next.js + Fumadocs) boots with one MDX page rendering a live Avero component
- [ ] `apps/replica` (Vite) boots
- [ ] Changesets initialised; `size-limit` configured
- [ ] CI pipeline runs all gates in §8.5 on every PR to `dev` and is green
- [ ] `docs/avero-conventions.md`, `docs/known-debts.md` and `docs/SECURITY.md` created
- [ ] Dependency versions pinned to current stable releases and recorded in `docs/avero-conventions.md`
...
**DoD:**
- [ ] `@avero/tokens/theme.css` defines every token in §5 as Tailwind v4 `@theme` variables (brand, semantic, categorical, chart, heatmap, gradients as utilities, micro font sizes, leading, shadows, radii, z-index, easings, animations)
- [ ] Reference `:root` names are available as a compatibility layer (`--primary` etc. aliasing the Avero tokens) so reference markup renders unchanged
- [ ] `tokens.json` (DTCG format) and `tokens.ts` are generated from a single source; a CI check fails if they drift
- [ ] **Coverage check:** a script asserts every value in `tokens.extracted.json` maps to a token (100%)
- [ ] `base.css`: body defaults, font smoothing, `min-width: 320px`, smooth scroll, `fancy`/`thin`/`hidden` scrollbar utilities, all 14 custom keyframes and animation utilities, `prefers-reduced-motion` override
